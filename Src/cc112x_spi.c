...
cc_tx_cmd (uint8_t CMDStrobe)
{
  ...
  /* chip select LOw */
  HAL_GPIO_WritePin (GPIOA, GPIO_PIN_4, GPIO_PIN_RESET); //CSn
  delay_us(5);
  /* Send-receive 1 byte */
...
//  uint8_t fifo_adr;
//
//  for(fifo_adr=0;fifo_adr<len;fifo_adr++){
//	  uint16_t temp_adr = FIFO_DMA_ACCESS;
//	  temp_adr |= fifo_adr;
//	  ret = cc_tx_wr_reg(temp_adr,data [fifo_adr]);
//  }
//
//  ret = numBytes;
		       ...
		       size_t timeout_ms)
{
  ...
  HAL_GPIO_WritePin(GPIOE, GPIO_PIN_2, GPIO_PIN_RESET);  //PA on
  ...
  while( gone + in_fifo < size) {
    ...
    if(first_burst){
      ...
      //SYSVIEW_PRINT("CC TX: Issue: %u GN: %u INFIFO: %u", issue_len,gone, in_fifo);
    }
  ...
  while (HAL_GetTick () - start_tick < timeout_ms) {
    ...
    if (tmp == 0) {
      timeout = 0;
      HAL_GPIO_WritePin(GPIOE, GPIO_PIN_2, GPIO_PIN_SET);  //PA off
      break;
    }
  }
...
//  ret = HAL_SPI_TransmitReceive (&hspi1, rx_spi_tx_buf, rx_tmp_buf, len + 1,
//				 COMMS_DEFAULT_TIMEOUT_MS);
//  delay_us(4);
//  HAL_GPIO_WritePin(GPIOE,GPIO_PIN_15, GPIO_PIN_SET);
