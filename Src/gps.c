...
extern UART_HandleTypeDef hspi1;
...
void UART_GPS_Receive_IT(UART_HandleTypeDef *huart) {
    ...
    c = (uint8_t) (huart->Instance->RDR & (uint8_t) 0x00FFU);
    ...
    get_gps_sentences();
    	...
    	if (gps_state_value->num_sat >= 3
    		...
    		if(vsi_flag==8){
    			...
    			update_adt7420 ();
    			gps_state_value->cctemp_gps = get_temp_adt7420();
...
void prepare_gps_pkt(_gps_state *state){
	...
	uint8_t full_pkt [DL_PKT_SIZE] = {'0'};
	memset(full_pkt, 0, DL_PKT_SIZE);
	...
	snprintf(cctemp_pkt, 5,"%f", state->cctemp_gps);
...
//	}
	...
	strcat(full_pkt, ",");
	strncat(full_pkt, cctemp_pkt,4);


	memset(spi_buf, 0, sizeof(spi_buf));
	tx_data(full_pkt,DL_PKT_SIZE,spi_buf,0,4000);
